...
// .wrangler/tmp/bundle-uYhSPA/checked-fetch.js
...
// .wrangler/tmp/bundle-uYhSPA/strip-cf-connecting-ip-header.js
...
router.post("/api/admin/seed", async (request, env) => {
  try {
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO items (name, description, category, replacement_value, risk_level, min_level_required) VALUES (?, ?, ?, ?, ?, ?)`).bind("LEGO Mindstorms EV3", "Complete robotics kit", "lego", 35e3, "low", 1),
      env.DB.prepare(`INSERT INTO items (name, description, category, replacement_value, risk_level, min_level_required) VALUES (?, ?, ?, ?, ?, ?)`).bind("ESP32 DevKit", "WiFi+BT microcontroller", "iot", 1500, "low", 1),
      env.DB.prepare(`INSERT INTO items (name, description, category, replacement_value, risk_level, min_level_required) VALUES (?, ?, ?, ?, ?, ?)`).bind("DJI Mini 3", "Compact drone", "electronics", 75e3, "high", 4),
      env.DB.prepare(`INSERT INTO items (name, description, category, replacement_value, risk_level, min_level_required) VALUES (?, ?, ?, ?, ?, ?)`).bind("Ender 3 V2", "3D Printer", "3d_printer", 3e4, "medium", 3)
    ]);
...
// .wrangler/tmp/bundle-uYhSPA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-uYhSPA/middleware-loader.entry.ts
